    ...
    <div className="account-panel">
      <div className="wallet_balance_card">
        ...
        <div className="wallet_balance_amount">
          ...
          <Image src={sarIcon} alt="SAR" width={34} height={34} className="currency_icon" />
        </div>

        <button type="button" className="wallet_topup_btn" onClick={onTopUp}>
          شحن الرصيد
        </button>
      </div>
        ...
        <div className="wallet_history_list">
          {transactions.map((tx) => (
            <div key={tx.id} className={`wallet_history_item ${tx.type}`}>
              ...
              <span
                className={`wallet_history_item_amount ${tx.type === 'payment' ? 'negative' : ''}`}
              >
